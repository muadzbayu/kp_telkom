...
class Absensi extends CI_Controller
{
    ...
    public function detail_absensi()
    {
        ...
        $data['pointer'] = "Dashboard"; 
        $data['sub_bread'] = "Daftar Absen Harian";
    ...
    public function export_pdf()
    {
        ...
        */
        ...
        $data = $this->detail_data_absen();
        //title dari pdf
        $filePdf = 'Absensi '.$data['mahasiswa']->nama . ' - ' . bulan($data['bulan']) . ' ' . $data['tahun'];
        $paper = 'A4';
        $orientation = "portrait";
        $html = $this->load->view('absensi/print_pdf', $data, true);
        $this->pdfgenerator->generate($html, $filePdf,$paper,$orientation );
    ...
    public function export_excel()
    {
        ...
        $excel->getProperties()
                ->setCreator('mahasiswa_kp')
                ->setLastModifiedBy('mahasiswa_kp')
                ->setTitle('Data Absensi')
                ->setSubject('Absensi')
                ->setDescription('Absensi' . $data['mahasiswa']->nama . ' bulan ' . bulan($data['bulan']) . ', ' . $data['tahun'])
        ...
        $excel->setActiveSheetIndex(0)->setCellValue('A1', 'Nama : ' . $data['mahasiswa']->nama);
        $excel->getActiveSheet()->mergeCells('A1:D1');
        $excel->getActiveSheet()->getStyle('A1')->getFont()->setBold(true);
        $excel->getActiveSheet()->getStyle('A1')->getFont()->setSize(12);

        $excel->setActiveSheetIndex(0)->setCellValue('A2', 'Divisi : ' . $data['mahasiswa']->nama_divisi);
        ...
        header('Content-Disposition: attachment; filename="Absensi ' . $data['mahasiswa']->nama . ' - ' . bulan($data['bulan']) . ' ' . $data['tahun'] . '.xlsx"'); // Set nama file excel nya
